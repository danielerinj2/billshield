...
class AdaptiveLetterGenerator:
    ...
    def generate_patient_summary(
        ...
        parts.append(
            ...
            "not legal judgments. Where charges appear far above benchmarks, "
            "you can legitimately request justification or revision from the "
        ...
        if low_conf:
            parts.append(f"LOW CONFIDENCE / REVIEW NEEDED ISSUES ({len(low_conf)}):")
            parts.append("These require clarification before challenging:")
            parts.append("")
            
            for issue in low_conf[:3]:
                desc = issue.get('description', '')
                parts.append(f"• {desc}")
                parts.append("  Action: Request procedure code / tariff clarification")
                parts.append("")
